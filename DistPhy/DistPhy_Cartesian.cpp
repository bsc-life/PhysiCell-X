...
namespace DistPhy
{
												...
												)
	{
		...
		//Exchange cell pos in buffers
		if (cells_to_left> 0){
			MPI_Isend(&snd_pos_left[0], cells_to_left, MPI_INT, X_LEFT, 3333, MPI_COMM_WORLD, &snd_req[2]);
		}
		if (cells_to_right > 0){
			MPI_Isend(&snd_pos_right[0], cells_to_right, MPI_INT, X_RIGHT, 4444, MPI_COMM_WORLD, &snd_req[3]);
		}

		if (cells_from_left > 0) {
			rcv_pos_left.resize(cells_from_left);
			MPI_Irecv(&rcv_pos_left[0], cells_from_left, MPI_INT, X_LEFT, 4444, MPI_COMM_WORLD, &rcv_req[2]);
		}
		if (cells_from_right > 0) {
			rcv_pos_right.resize(cells_from_right);
			MPI_Irecv(&rcv_pos_right[0], cells_from_right, MPI_INT, X_RIGHT, 3333, MPI_COMM_WORLD, &rcv_req[3]);
		}

		if(cells_from_left > 0)
		MPI_Wait(&rcv_req[2], MPI_STATUS_IGNORE);
		
		if(cells_from_right > 0)
		MPI_Wait(&rcv_req[3], MPI_STATUS_IGNORE);

		if (cells_to_left > 0) {
			MPI_Wait(&snd_req[2], MPI_STATUS_IGNORE);
		}
		if (cells_to_right > 0) {
			MPI_Wait(&snd_req[3], MPI_STATUS_IGNORE);
		}
	}
    }
}
