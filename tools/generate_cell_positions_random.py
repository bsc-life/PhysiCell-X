...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate randomly distributed cell positions inside a cube and "
...
def generate_positions(num_cells: int, side_um: float, rng):
    if num_cells <= 0:
...
def main() -> int:
    ...
    if args.seed is not None:
        ...
        rng = random
    try:
        positions = generate_positions(args.num_cells, args.side_um, rng)
