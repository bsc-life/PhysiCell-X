...
//Jose Luis notes
//1.9: Gaurov is packing the class network by packing the vector<Node*> and the symbolTable (esta dentro de network?)
//1.14: Hay que añadir algo más?? 
...
void MaBoSSNetwork::pack(std::vector<char>& buffer, int& len_buffer, int& position){
	...
	pack_buff(t_nodes.size(),buffer, len_buffer, position);
	int temp_int;
...
void MaBoSSNetwork::print_state_after_unpack() const {
    ...
    for (size_t i = 0; i < t_nodes.size(); ++i) {
        ...
        std::cout << "  Node " << i << " (" << t_nodes[i]->getName() << "): "
